...
const SearchEngineMobile = () => {
  ...
  const [visibilityMoreFields, setVisibilityMoreFields] = useState(false); // RESPONSIBLE FOR DISPLAY LIST OF ADDITIONAL FIELDS
  const [initialRender, setInitialRender] = useState(true);
  const [showOptions, setShowOptions] = useState(false); // RESPONSIBLE FOR SHOWING OPTIONS OF FORM FIELD
  const [selectType, setSelectType] = useState(); // STORE VALUE OF CURRENT CLICKED FORM FIELD
  // REFS
  ...
  const submitSearchEngineHandler = (event) => {
    ...
    setVisibilityMoreFields(false);
  };

  useEffect(() => {
    // RESPONSIBLE FOR SHOWING OR HIDING OPTIONS OF FORM FIELD WHEN USER PICK IT
    ...
    if (!initialRender) {
      ...
      if (!showOptions) {
        if (additionalFields) {
          province.current.nextSibling.style.display = "none";
          market.current.nextSibling.style.display = "none";
          property.current.nextSibling.style.display = "none";
          transaction.current.nextSibling.style.display = "none";
        }
  ...
  const showOptionsHandler = (event) => {
    // RESPONSIBLE FOR STATE MANAGEMENT OF SHOWING OR HIDING OPTIONS OF FORM FIELD
    setShowOptions((previousState) => !previousState);
    setSelectType(event.target.nextSibling); // STORE FORM FIELD
  };

  const moreFieldsHandler = () => {
    // RESPONSIBLE FOR STATE MANAGEMENT OF VISIBILITY OF ADDITIONAL FORM FIELDS
    setVisibilityMoreFields((previousState) => !previousState);
  };
  let additionalFields;

  if (visibilityMoreFields) {
    // ADDITIONAL FORM FIELS VISIBILE ONLY IF USER CLICK MORE PROPERTIES BUTTON
    additionalFields = (
      <Fragment>
        <div className="searchEngineMobile__selectContainer">
          <div ref={province} onClick={showOptionsHandler} className="selectContainer__field">
        ...
        <div className="searchEngineMobile__selectContainer">
          <div ref={market} onClick={showOptionsHandler} className="selectContainer__field">
  ...
  return (
    <form onSubmit={submitSearchEngineHandler} className={darkMode ? "searchEngineMobile searchEngineMobile--styling searchEngineMobile--dark" : "searchEngineMobile searchEngineMobile--styling searchEngineMobile--bright"}>
      ...
      <div className="searchEngineMobile__selectContainer">
        <div ref={transaction} onClick={showOptionsHandler} className="selectContainer__field">
